...
class Transformer:
    ...
    def _clean_structure(self, ticker):
        file_path = self.data_dir / f"{ticker}.csv"
...
class Analyzer:
    ...
    @classmethod
    def analyze_data(cls, data_dir, input_filename="merged_stock_data.csv", output_filename="merged_stock_data_with_analysis.csv"):

        df = load_csv(input_filename, data_dir)
...
class Visualizer:
    ...
    def visualize(path, tickers):
        df = load_csv("merged_stock_data_with_analysis.csv", path)
    ...
    def plot_closing_prices(self, df, ax=None):
        ...
        # The x-axis configuration will be applied by _finalize_plot via the style_plot decorator
    ...
    def _plot_scatter(self, ax, data, ticker, color, marker, label_suffix):
    ...
    def _plot_crossovers(self, ax, stock_df, ticker, color):
